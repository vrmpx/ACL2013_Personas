...
def partokify(jsent):
    row = []
    row.append(' '.join(t[0] for t in jsent['tokens']))#.encode('utf8'))
    row.append(' '.join(t[1] for t in jsent['tokens']))#.encode('utf8'))
    row.append(' '.join(t[2] for t in jsent['tokens']))#.encode('utf8'))
    row.append(' '.join(t[3] for t in jsent['tokens']))#.encode('utf8'))
...
class Entity(dict):
  ...
  def __gt__(self, other):
    return self['first_mention']>other['first_mention']
...
def corexmls_from_tsv(linegen):
    for line in linegen:
        parts = line.split('\t')
        if len(parts)<2:
            print("skipping line starting with: {}".format(line[:50]), file=sys.stderr)
            continue
        docid = '\t'.join(parts[:-1])
        data = parts[-1]
        yield docid, data
...
if __name__=='__main__':
    ...
    for docid,data in corexmls_autodetect():
        try:
            ...
            print("XML parse failed on doc: {}".format(docid), file=sys.stderr)
            continue
